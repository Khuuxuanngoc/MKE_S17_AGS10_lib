...
// This is a library for the AGS10 sensor
// It measures the total volatile organic compounds (TVOC) in the air
...
class AGS10 { 
  ...
  public:
    ...
    AGS10();  // Constructor 
    void begin(TwoWire *wire = &Wire, uint8_t i2cAddr = AGS10_ADDR); // Initialize the sensor with optional TwoWire instance
    ...
    uint32_t readResist(); // Read the current resistance of the sensor in 0.1 kOhm
    int readVersion();  // Read the sensor's current firmware version 
    uint32_t readTVOC(); // Read the TVOC value in ppb (up to 99999)
    bool isReady(); // Check if the sensor is ready (not busy)
    uint8_t Calc_CRC8(uint8_t *dat, uint8_t Num); 


  private: 

    uint32_t _tvoc; // Variable to store the TVOC value 
    int _version; // Variable to store the version value 
    uint32_t _resist;  // Variable to store the resistance value
    byte _crc;
    TwoWire *_wire; // Pointer to the I2C bus
    uint8_t _i2cAddr; // Current I2C address
    uint32_t _lastTVOCReadTime;
    uint32_t _lastResistReadTime;
